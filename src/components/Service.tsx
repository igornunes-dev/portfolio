...
export const Service = () => {
  ...
  useEffect(() => {
    ...
    return (  
        ...
        <motion.article className="mt-6 flex max-w-4xl overflow-hidden cursor-pointer" whileTap={{cursor: "grabbing"}} ref={carrousel}>
            <motion.div className="flex flex-col gap-2 md:flex-row" drag="x"  dragConstraints={{right: 0, left: -width}}>
                {services.map((service, index) => (
                <motion.div 
                className="rounded-lg p-6 bg-blue-500 text-white h-[200px] w-[400px] mx-2 item flex md:flex-col" 
                key={`service-${index}`}>   
